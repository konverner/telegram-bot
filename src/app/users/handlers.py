...
from .service import read_user, upsert_user
...
def register_handlers(bot):
    ...
    def read_user_data(message: Message, data: dict):
        ...
        if user_data.isdigit():
            ...
            if not retrieved_user:
                bot.send_message(
                    user.id,
                    app_strings[user.lang].username_not_found.format(username=user_data),
                )
                return
    ...
    def grant_admin_handler(call, data: dict):
        ...
        db_session = data["db_session"]
        upsert_user(db_session, user_id=grant_admin_user_id, role_id=0)
        bot.send_message(
            user.id,
            app_strings[user.lang].add_admin_confirm.format(user_id=grant_admin_user_id),
            parse_mode="Markdown",
        )
    ...
    def block_user_handler(call, data: dict):
        ...
        db_session = data["db_session"]
        upsert_user(db_session, user_id=block_user_id, is_blocked=True)
        bot.send_message(
            user.id,
    ...
    def revoke_admin_handler(call, data: dict):
        """Handler to revoke admin rights from a user"""
        user = data["user"]
        revoke_admin_user_id = call.data.split("_")[2]
        db_session = data["db_session"]
        upsert_user(db_session, user_id=revoke_admin_user_id, role_id=1)
        bot.send_message(
            user.id,
            app_strings[user.lang].revoke_admin_confirm.format(user_id=revoke_admin_user_id),
            parse_mode="Markdown",
        )
