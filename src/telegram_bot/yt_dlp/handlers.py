...
from telebot.states.sync.context import StateContext  # Import StateContext if using sync
...
def register_handlers(bot: TeleBot) -> None: # Pass state_storage if needed
    ...
    def start_yt_dlp(call: types.CallbackQuery, data: Dict[str, Any]) -> None:
        ...
        bot.edit_message_text(
            ...
            reply_markup=create_cancel_button(lang, callback_data="menu") # Cancel goes back to main menu
        )

    # Handler to automatically detect YouTube links in any message (outside specific states)
    @bot.message_handler(func=lambda message: re.search(YOUTUBE_URL_PATTERN, message.text) is not None, content_types=['text'], state=None)
    def handle_youtube_link(message: types.Message, data: Dict[str, Any]) -> None:
        ...
        if match:
            ...
            bot.send_message(
                message.chat.id,
                strings[lang].get("detected_yt_link", "YouTube link detected! Choose format to download:"), # Use .get for safety
                reply_markup=create_format_selection_markup(lang)
            )
        # If no match (shouldn't happen due to the func filter, but good practice)
        else:
             logger.warning(f"YouTube link handler triggered for message without link: {message.text}")
    ...
    @bot.callback_query_handler(func=lambda call: call.data.startswith("ydl_format_"), state=YouTubeDLState.awaiting_format)
    def process_format_selection(call: types.CallbackQuery, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if downloaded_file_path and downloaded_file_path.exists():
                ...
                with open(downloaded_file_path, 'rb') as file:
                    bot.send_document(
                        ...
                        file, 
                        caption=strings[lang].download_success.format(filename=downloaded_file_path.name)
                    )
            ...
            logger.exception(f"Unexpected error during download/send for URL {url}: {e}")
            bot.send_message(
                call.message.chat.id,
                strings[lang].download_error.format(error="An unexpected error occurred."),
                reply_markup=create_back_to_menu_button(lang)
            )
        finally:
    ...
    @bot.callback_query_handler(func=lambda call: call.data == "ydl_cancel", state=YouTubeDLState.awaiting_format)
