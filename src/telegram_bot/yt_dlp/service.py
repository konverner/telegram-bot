...
def download_youtube_content(url: str, download_type: str, output_path: Path = DEFAULT_DOWNLOAD_PATH) -> Path:
    """
    ...
    if download_type == "video":
        ...
        ydl_opts['merge_output_format'] = 'mp4'  # Request mp4 container if merging is needed
    ...
    try:
        ...
        logger.error(f"yt-dlp download error for {url} ('{sanitized_title}', ID: {video_id}): {e}", exc_info=True)
        raise DownloadError(f"Failed to download: {e}")
    except Exception as e:
        # Catching potential errors during download/postprocessing
        logger.error(f"An unexpected error occurred during download/processing for {url} ('{sanitized_title}', ID: {video_id}): {e}", exc_info=True)
    ...
    if expected_final_path.exists():
        ...
        logger.warning(f"Expected file {expected_final_path} not found. Scanning {output_path} for files matching pattern '{glob_pattern}'...")
        found_files = list(output_path.glob(glob_pattern))

        if not found_files:
            logger.error(f"Download process completed but no output file found for '{sanitized_title}' (ID: {video_id}) in {output_path}.")
            raise DownloadError(f"Could not locate the final downloaded file for '{sanitized_title}' (ID: {video_id}).")
        ...
        if potential_file:
            ...
            logger.warning(f"File with expected extension {final_expected_ext} not found for title '{sanitized_title}'. "
