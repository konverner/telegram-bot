...
def send_scheduled_message(
    bot, user_id: int, media_type: str,
...
def register_handlers(bot):
    ...
    def query_handler(call: types.CallbackQuery, data: dict):
        ...
        bot.send_message(
            user.id, strings[user.lang].public_message.menu.title, reply_markup=create_keyboard_markup(user.lang)
        )
    ...
    def create_public_message_handler(call: types.CallbackQuery, data: dict):
        ...
        sent_message = bot.send_message(
            user.id,
            strings[user.lang].public_message.enter_datetime_prompt.format(timezone=config.timezone),
    ...
    def get_datetime_input(message, bot, user):
        ...
        try:
            ...
            if user_datetime_localized < datetime.now(timezone):
                ...
                sent_message = bot.send_message(
                    user.id,
                    strings.enter_datetime_prompt[user.lang].format(timezone=config.timezone),
                    reply_markup=create_cancel_button(user.lang),
                )
                bot.register_next_step_handler(sent_message, get_datetime_input, bot, user)
                return
            ...
            sent_message = bot.send_message(
                user.id, strings.record_message_prompt[user.lang], reply_markup=create_cancel_button(strings, user.lang)
            )
